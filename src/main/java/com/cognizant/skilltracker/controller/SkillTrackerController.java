...
public class SkillTrackerController {
    ...
    public ResponseEntity<String> addProfile(@PathVariable("userId") String userId, @RequestBody List<Skill> skills) {
        ...
        FseDocument fseDocument = profileService.getProfile(userId).get();
        List<SkillDocument> documentList = profileService.getSkills(userId);
        if (fseDocument == null) {
            return new ResponseEntity<String>("UserId Invalid Or Profile Not Found", HttpStatus.NOT_FOUND);
        }
        if (Duration.between(LocalDateTime.now(), fseDocument.getLastUpdatedDateTime()).toDays() > -10) {
            return new ResponseEntity<String>("Profile Update Allowed Only After 10 Days", HttpStatus.OK);
        }
        for (SkillDocument skill : documentList) {
            Skill updatedSkill = skills.stream().filter(s -> s.getSkill().equalsIgnoreCase(skill.getSkillName()))
                    .findFirst().get();
        ...
        fseDocument.setLastUpdatedDateTime(LocalDateTime.now());
        rabbitMQSender.sendFseProfile( FseSkillTracker.builder().skillDocumentList(documentList).fseDocument(fseDocument).build());
        return new ResponseEntity<String>(userId, HttpStatus.OK);
    }
